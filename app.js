/* ================================================================
   ...
   IDs sincronizados para Selenium IDE:
     CP-001: dashboardTitle, dashboard
     CP-002: loginError
     CP-003: loginForm, loginTitle  (en index.html)
     CP-004: eventoSuccess, mensajeEvento, listaEventos
     CP-005: inscripcionSuccess, mensajeInscripcion, listaParticipantes
     CP-006: errorInscripcion  → "Todos los campos son obligatorios"
     CP-007: reporteMensaje, mensajeReporte, tablaReporte
   ================================================================ */
(function () {
    'use strict';
    ...
    /* Muestra alerta — si hay un ID espejo lo sincroniza también */
    var MIRROR = {
        mensajeEvento:    'eventoSuccess',
        eventoSuccess:    'mensajeEvento',
        mensajeInscripcion: 'inscripcionSuccess',
        inscripcionSuccess: 'mensajeInscripcion',
        mensajeReporte:   'reporteMensaje',
        reporteMensaje:   'mensajeReporte'
    };
    ...
    function showAlert(id, msg) {
        ...
        /* sincronizar ID espejo para Selenium */
        var mirror = MIRROR[id];
        if (mirror) {
            var m = el(mirror);
    ...
    /* Oculta mensaje tras ms milisegundos — tiempo largo para que Selenium lo lea */
    function autoHide(id, ms) {
        setTimeout(function () { hideAlert(id); }, ms || 6000);
    }

    function clearAllAlerts() {
        ['errorEvento','mensajeEvento','eventoSuccess',
         'errorInscripcion','mensajeInscripcion','inscripcionSuccess',
         'errorReporte','mensajeReporte','reporteMensaje',
         'mensajeExportar'].forEach(function (id) {
            var e = el(id);
    ...
    function guardarEvento(e) {
        ...
        /* CP-004: texto exacto para assertText */
        showAlert('mensajeEvento', 'Evento creado exitosamente');
        /* autoHide largo: Selenium tiene tiempo de leer el assertText */
        autoHide('mensajeEvento', 8000);

        limpiarFormEvento();
        renderEventos();
        /* Cierra form después de que Selenium haya leído el mensaje */
        setTimeout(closeFormEvento, 8200);
    }
    ...
    function renderEventos() {
        ...
        eventos.forEach(function (ev) {
            ...
            html += '<div class="evento-card" data-evento-id="' + ev.id + '">'
                ...
                +   '<div class="meta-item meta-item--full"><strong>Descripción</strong>' + esc(ev.descripcion) + '</div>'
                + '</div>'
                + '</div>';
        });
    ...
    function inscribirParticipante(e) {
        ...
        /* CP-006: validar si los tres campos de datos están vacíos */
        if (!nombre && !email && !idNum) {
            showAlert('errorInscripcion', 'Todos los campos son obligatorios');
            return;
        }
        if (!eventoId) {
            showAlert('errorInscripcion', 'Todos los campos son obligatorios'); return;
        }
        if (!nombre) {
            showAlert('errorInscripcion', 'Todos los campos son obligatorios');
        ...
        /* CP-005: texto exacto para assertText */
        showAlert('mensajeInscripcion', 'Inscripción exitosa');
        autoHide('mensajeInscripcion', 8000);
    ...
    function generarReporte() {
        ...
        if (!evento) {
            showAlert('errorReporte', 'El evento seleccionado no existe.'); return;
        }
        ...
        if (!lista.length) {
            ...
            tablaEl.innerHTML = '<table>'
                + '<thead><tr><th>#</th><th>Nombre Completo</th><th>Correo Electrónico</th>'
                + '<th>Identificación</th><th>Fecha de Inscripción</th></tr></thead>'
                + '<tbody>' + rows + '</tbody></table>';
        }

        el('contenedorReporte').classList.remove('d-none');

        /* CP-007: texto exacto para assertText en reporteMensaje y mensajeReporte */
        showAlert('mensajeReporte', 'Reporte generado correctamente');
        autoHide('mensajeReporte', 8000);
    }
    ...
    function exportarReporte() {
        ...
        lista.forEach(function (p, i) {
            var f = new Date(p.inscritoEn).toLocaleString('es-CO');
            lines.push((i + 1) + ',"' + p.nombre + '","' + p.email
                + '","' + p.identificacion + '","' + f + '"');
        });
        ...
        autoHide('mensajeExportar', 8000);
    }

    /* ── Helper HTML ────────────────────────────────────────────── */
    function buildEmptyState(svgPath, title, desc) {
        return '<div class="empty-state">'
            + '<div class="empty-state-icon"><svg viewBox="0 0 24 24" fill="none"'
            + ' stroke="currentColor" stroke-width="1.5">' + svgPath + '</svg></div>'
